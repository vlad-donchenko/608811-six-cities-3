...
it(`Should render <OfferNeighbourhood/>`, () => {
  const tree = renderer.create(<OfferNeighbourhood additionalClass={OfferPrefix.NEAR_PLACES_CARD} renderOffer={() => {}} offers={offers.slice(0, MAX_NEIGHBOURHOOD_OFFERS)} onTitleClick={() => {
  }}/>).toJSON();
